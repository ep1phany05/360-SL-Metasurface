""" 
减少特征通道数（Channel Reduction）：将特征提取和代价体（Cost Volume）的通道数从 34 减少到 16。
减少网络层数（Layer Pruning）：
    FeatureExtractor 中的残差块（Residual Blocks）数量从 5 减少到 2。
    3D代价体滤波（3D Filter）的层数从 5 减少到 2。
    边缘细化模块（Refinement）的卷积层数和膨胀卷积深度减半。
"""
...
class EdgeAwareRefinement(nn.Module):
    ...
    def __init__(self, in_channel):
        ...
        # [Modified] Reduced channels from 32 to 16
        self.edge_in = nn.Sequential(
            convbn(4, 16, kernel_size=3, stride=1, pad=1, dilation=1), nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.edge_filter = nn.ModuleList()
        # [Modified] Reduced layers from 5 to 2
        for i in range(2):
            self.edge_filter.append(BasicBlock(16, 16, stride=1, downsample=None, pad=1, dilation=1))

        self.edge_out = nn.Conv2d(16, 1, kernel_size=3, stride=1, padding=1)

        self.conv2d_feature = nn.Sequential(
            convbn(2, 16, kernel_size=3, stride=1, pad=1, dilation=1), nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.residual_astrous_blocks = nn.ModuleList()
        # [Modified] Reduced astrous blocks from [1, 2, 3, 4, 3, 2, 1] to [1, 2, 1]
        astrous_list = [1, 2, 1]
        for di in astrous_list:
            self.residual_astrous_blocks.append(BasicBlock(16, 16, stride=1, downsample=None, pad=1, dilation=di))

        self.conv2d_out = nn.Conv2d(16, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, low_disparity, corresponding_rgb):
        ...
        output = self.conv2d_feature(torch.cat([twice_disparity, edge_map], dim=1))

        for astrous_block in self.residual_astrous_blocks:
            output = astrous_block(output)

        return nn.ReLU(inplace=True)(torch.squeeze(twice_disparity + self.conv2d_out(output) * 0.01, dim=1))
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, k, in_channel=3, out_channel=16):  # [Modified] Default out_channel changed
        super().__init__()
        self.k = k
        ...
        for _ in range(k):
            self.downsample.append(nn.Conv2d(self.in_channel, self.out_channel, kernel_size=5, stride=2, padding=2))
            self.in_channel = self.out_channel

        self.residual_blocks = nn.ModuleList()
        # [Modified] Reduced blocks from 5 to 2
        for _ in range(2):
            self.residual_blocks.append(BasicBlock(self.out_channel, self.out_channel, stride=1, downsample=None, pad=1, dilation=1))
...
class DepthEstimator(nn.Module):
    ...
    def __init__(self, pano_cam, fish_cam, device, opt):
        ...
        # [Modified] Reduced feature channels from 34 to 16
        self.num_features = 16

        self.filter = nn.ModuleList()
        # [Modified] Reduced 3D filter layers from 5 to 2
        for _ in range(2):
            self.filter.append(
                nn.Sequential(
    ...
    def forward(self, img_list):
        ...
        for i, j in [(0, 1)]:  #, (1, 0)]
            ...
            intensity = torch.nn.functional.grid_sample(
                target_img, 2 * grid - 1, mode='bilinear', padding_mode='zeros'
            )  # B x C x R x H x W
            intensity = torch.nn.functional.interpolate(
                intensity.reshape(B, -1, H, W), size=(H // 4, W // 4), mode='bilinear'
            ).reshape(B, -1, R, H // 4, W // 4)
