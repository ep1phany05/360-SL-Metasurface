...
class EdgeAwareRefinement(nn.Module):
    ...
    def __init__(self, in_channel):
        ...
        self.edge_in = nn.Sequential(
            convbn(4, 32, kernel_size=3, stride=1, pad=1, dilation=1), nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )

        self.edge_filter = nn.ModuleList()
        for i in range(5):
            self.edge_filter.append(BasicBlock(32, 32, stride=1, downsample=None, pad=1, dilation=1))

        self.edge_out = nn.Conv2d(32, 1, kernel_size=3, stride=1, padding=1)

        self.conv2d_feature = nn.Sequential(
            convbn(2, 32, kernel_size=3, stride=1, pad=1, dilation=1), nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
...
# 		self.out_channel = out_channel
#
# 		# For storing features at different levels
# 		self.downsample_features = []
#
# 		for _ in range(k):
...
# 		for d in self.downsample:
# 			output = d(output)
# 			self.downsample_features.append(output)  # Capture features after each downsample
#
# 		for block in self.residual_blocks:
# 			output = block(output)
#
# 		output = self.conv_alone(output)
# 		self.downsample_features.append(output)
#
# 		return self.downsample_features, output
