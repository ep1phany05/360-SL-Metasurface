...
def test_with_metrics(opt, model: nn.Module):
    ...
    for i, data in enumerate(dataloader_test):
        ...
        for ci in range(n_eval):
            ...
            valid = valid * (depth_pred_m > min_depth).float() * (depth_pred_m < max_depth).float()
            ...
            save_colormap_png(depth_root / cn / "error" / f"{name}.png", de_n, cmap="inferno")
            ...
            if len(normal_im_list) > ci and normal_im_list[ci] is not None:
                ...
                save_colormap_png(normal_root / cn / "error" / f"{name}_angular.png", ang_vis, cmap="inferno")
