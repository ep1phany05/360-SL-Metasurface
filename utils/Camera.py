...
class Camera(nn.Module):
    ...
    def __init__(
        self, cam_type, fov, focal_length, location, rotation, name, sensor_width, sensor_height, resolution_x, resolution_y, device
    ):
        super().__init__()
...
class FisheyeCam(Camera):
    ...
    def __init__(self, opt, location, rotation, name, device, config_path):
        super().__init__(
            "Fisheye", opt.fov, opt.focal_length, location, rotation, name, opt.sensor_width, opt.sensor_height, opt.fisheye_resolution_x,
            opt.fisheye_resolution_y, device
        )
        self.device = device
        ...
        # 将关键矩阵注册为buffer，不主动to(device)，DDP会处理
        self.register_buffer('affine_mat', torch.tensor([[self.c, self.d], [self.e, 1.]]))
        self.register_buffer('inv_affine_mat', torch.inverse(self.affine_mat))
        self.register_buffer('rot_axis', torch.tensor([[1, 0, 0], [0, -1, 0], [0, 0, -1]], dtype=torch.float32))
    ...
    def whole_pixel2world(self):
        ...
        x = torch.arange(0, self.resolution_x, device=self.device)
        y = torch.arange(0, self.resolution_y, device=self.device)
    ...
    def pixel2world(self, input_pixels):
        ...
        # input_pixels: tensor, 已经在正确device上
        device = input_pixels.device
        # pixels = (input_pixels) - torch.Tensor([[self.center_x], [self.center_y]]).to(self.device)
        pixels = input_pixels - torch.tensor([[self.center_x], [self.center_y]], device=device)
        ...
        affine_mat = self.affine_mat.to(device)
        inv_affine_mat = self.inv_affine_mat.to(device)
        sensor_coord = torch.matmul(inv_affine_mat, pixels)

        r = torch.sqrt(sensor_coord[0] ** 2 + sensor_coord[1] ** 2)  # N
        r_poly = torch.stack([r ** i for i in range(self.num_poly)])

        poly_coef = self.poly_coef.to(device)
        z = -1 * torch.matmul(poly_coef, r_poly)
        ...
        rot_axis = self.rot_axis.to(device)
...
class PanoramaCam(Camera):
    ...
    def __init__(self, opt, location, rotation, name, device):
        super().__init__(
            "Pano", opt.fov, opt.focal_length, location, rotation, name, opt.sensor_width, opt.sensor_height, opt.pano_resolution_x,
            opt.pano_resolution_y, device
        )
