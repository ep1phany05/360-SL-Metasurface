...
def _draw_orientation_circles_and_arrows(
    ...
    # 改动：arrow_scale 现在按“直径”缩放（最终箭头长度 = circle_diameter * arrow_scale）
    ...
    # 改动：默认更深颜色
    arrow_color: str = "0.0",
    # 改动：默认更粗（单位是 dots/pixels，因为 units="dots"）
    arrow_width: float = 2.2,
    # 改动：箭头头部更大一些（这些是 width 的倍数）
    headwidth: float = 4.5,
    headlength: float = 6.5,
    headaxislength: float = 5.5,
):
    """
    ...
    改动要点：
    - 圆形框可关闭（draw_circles=False）
    - 箭头使用 pivot='middle'，以圆心为中点贯穿（长度按“直径”计算）
    """
    h, w = phase_map.shape
    ...
    # =========================
    # 改动：箭头长度从“半径”改为“直径”
    # 以前：arrow_len = radius * arrow_scale
    # 现在：arrow_len = circle_diameter * arrow_scale
    # 再配合 pivot="middle" => 箭头以圆心为中点贯穿
    # =========================
    arrow_len = float(circle_diameter) * float(arrow_scale)
...
def visualize_phase_maps_split_save(
    ...
    Save three figures:
      ...
      (B) 1xN orientation glyphs (arrows, angle=deg(phase)%360)
    ...
    for i, (pm, nm) in enumerate(zip(phase_maps, valid_names)):
        ...
        _draw_orientation_circles_and_arrows(
            ...
            # =========================
            # 改动：箭头“直径长度”+“贯穿圆心”
            # arrow_scale=1.0 表示等于 circle_diameter（默认 0.92*stride）
            # =========================
            arrow_scale=1.0,
            # =========================
            # 改动：加粗 + 更清晰
...
if __name__ == "__main__":
    file_list = [
        "/data/wudelong/result/360-sl-metasurface/runs/260125-172954/phase_best_epoch_591.npy",
        "/data/wudelong/result/360-sl-metasurface/runs/260125-172900/phase_best_epoch_988.npy",
    ]
    name_list = ["NORM", "Pixelwise"]
    save_root = "./vis/260125/phasemap_v2"
    ...
    visualize_phase_maps_split_save(
        ...
        dpi_ori=400,
        cell_px=20,
        use_legacy_physics=True,
        legacy_extra_square=None,
        fourier_max_res=500,
