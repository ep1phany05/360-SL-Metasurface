...
@torch.no_grad()
def check_occlusion_consistency(
    depth_src: torch.Tensor,
    depth_tgt: torch.Tensor,
    cam_src,
    cam_tgt,
    R_src_to_tgt: torch.Tensor,
    t_src_to_tgt: torch.Tensor,
    valid_mask_src: torch.Tensor,
    outside_as_occluded: bool = True,
    margin: float = 0.05
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Use geometric consistency (Stereo Check) to determine occlusion.
    Project Source Depth to Target View and compare with Target Predicted Depth.
    If Project(Depth_src) > Depth_tgt + margin, then it is occluded.

    Returns:
      occ_prob: (1,1,H,W) probability (here binary)
      occ_bin:  (1,1,H,W) binary occlusion mask (1=occluded)
    """
    B, _, H, W = depth_src.shape

    # 1. Back-project Source pixels to 3D
    rays_src = cam_src.get_whole_pts().reshape(3, H, W).to(depth_src.device).float()
    P_src = (depth_src[0, 0] * rays_src).reshape(3, -1)  # 3xN

    # 2. Transform to Target Camera Coords
    P_in_tgt = (R_src_to_tgt @ P_src) + t_src_to_tgt.reshape(3, 1)  # 3xN

    # 3. Project to Target Image Plane
    pix_tgt = cam_tgt.world2pixel(P_in_tgt)
    u_tgt = pix_tgt[0]
    v_tgt = pix_tgt[1]
    vproj = pix_tgt[2]  # 1 if within cam2 FOV and image bounds

    # Depth in Target Frame (calculated from source projection)
    d_src_proj = torch.linalg.norm(P_in_tgt, dim=0).clamp_min(1e-8)

    # 4. Sample Target Predicted Depth at projected coordinates
    # Normalize coordinates to [-1, 1] for grid_sample
    # u, v from world2pixel are in pixel coords [0, W-1], [0, H-1]
    u_norm = (u_tgt / (W - 1)) * 2 - 1
    v_norm = (v_tgt / (H - 1)) * 2 - 1

    # shape: (1, 1, N, 2) -> grid_sample expects (N, H_out, W_out, 2) or (1, 1, N, 2) for flat sampling
    # Here we sample all N points.
    grid = torch.stack([u_norm, v_norm], dim=-1).reshape(1, 1, -1, 2)  # 1x1xNx2

    # Sample from Target Depth Map
    # mode='nearest' avoids interpolation artifacts at depth discontinuities
    d_tgt_sampled = F.grid_sample(depth_tgt, grid, mode='nearest', padding_mode='border', align_corners=True).reshape(-1)

    # 5. Consistency Check
    # diff = Projected_Depth - Target_Depth
    # If Projected is significantly behind Target Surface, it is occluded.
    diff = d_src_proj - d_tgt_sampled

    # Occlusion condition
    is_occluded = (diff > margin) & (vproj > 0.5)

    if outside_as_occluded:
        # Points projecting outside Target FOV are treated as occluded (or unknown/invisible)
        is_occluded = is_occluded | (vproj < 0.5)

    # Reassemble to (H, W)
    occ_bin_flat = torch.zeros((H * W), device=depth_src.device, dtype=torch.float32)
    # Only update valid source pixels to avoid noise
    src_valid_flat = valid_mask_src.reshape(-1) > 0.5
    occ_bin_flat[src_valid_flat] = is_occluded[src_valid_flat].float()

    occ_bin = occ_bin_flat.reshape(1, 1, H, W)
    occ_prob = occ_bin.clone()  # Binary for consistency

    return occ_prob, occ_bin
...
def test_with_metrics(opt, model: nn.Module):
    ...
    for i, data in enumerate(dataloader_test):
        ...
        for ci in range(n_eval):
            ...
            # --- Occlusion Metrics (Cam1 & Cam2) ---
            # 只要 GT 数据里有当前视角的遮挡图，我们就进行评估
            if len(occ_im_list) > ci and occ_im_list[ci] is not None:
                gt_occ = normalize_gt_occ(occ_im_list[ci].to(device))
                if gt_occ.shape[-2:] != (H, W):
                    gt_occ = F.interpolate(gt_occ, size=(H, W), mode="nearest")

                # 确定配对相机 (Stereo Pair)
                # 假设 inv_preds 列表长度为 >= 2 则有配对视角
                pair_ci = 1 - ci  # 0->1, 1->0

                # 获取相对位姿 src -> tgt
                if ci == 0:
                    # Cam1 -> Cam2
                    R_rel, t_rel = R_21, t_21
                    cam_src, cam_tgt = cam1, cam2
                else:
                    # Cam2 -> Cam1: T_12 = inv(T_21)
                    # T_21 = [R|t] -> T_12 = [R.T | -R.T @ t]
                    R_rel = R_21.t()
                    t_rel = -R_rel @ t_21
                    cam_src, cam_tgt = cam2, cam1

                # 准备当前预测深度 (depth_pred_m 已经在上面计算好了)
                depth_src_m = depth_pred_m

                # 尝试获取配对视角的预测深度
                depth_tgt_m = None
                if pair_ci < len(inv_preds):
                    # 获取配对视角的预测，并转为米
                    inv_pred_tgt = to_nchw(inv_preds[pair_ci].to(device)).float()
                    d_tgt_raw = to_depth_m_from_inv_pred(inv_pred_tgt)

                    # 注意：如果启用了 depth_align (median scaling)，我们也需要对配对视角的深度进行对齐，
                    # 否则尺度不一致会导致投影误差。
                    # 这里为了简化，我们尝试获取配对视角的 GT 深度来进行对齐
                    if pair_ci < len(depth_im_list):
                        gt_tgt_raw = to_nchw(depth_im_list[pair_ci].to(device)).float()
                        gt_tgt_m = to_depth_m_from_gt(gt_tgt_raw, depth_scale_m, gt_unit=gt_unit)
                        # 使用同样的 mask 逻辑
                        valid_tgt = mask_fish * (gt_tgt_m > min_depth).float() * (gt_tgt_m < max_depth).float()
                        depth_tgt_m = align_depth_scale(d_tgt_raw, gt_tgt_m, valid_tgt, mode=depth_align)
                    else:
                        # 如果没有配对 GT (不太可能)，则直接使用 raw 预测
                        depth_tgt_m = d_tgt_raw

                # 计算预测遮挡图
                occ_eval_mask = valid
                if depth_tgt_m is not None:
                    # 方案 A: 双视角一致性检查 (Stereo Consistency) - 更准确
                    occ_prob, occ_bin = check_occlusion_consistency(
                        depth_src_m,
                        depth_tgt_m,
                        cam_src,
                        cam_tgt,
                    ...
                    # 方案 B: 单视角自遮挡 (Self-Occlusion) - 回退方案
                    if i == 0:
                        print(f"[WARN] Cam{pair_ci+1} depth missing, utilizing self-occlusion for Cam{ci+1}")
                    occ_prob, occ_bin, occ_eval_mask = occlusion_cam1_seen_from_cam2_no_cam2_depth(
                        depth_src_m,
                        cam_src,
                        cam_tgt,
                        R_rel,
                ...
                if occ_gt_sem == "auto" and i == 0 and ci == 0:
                    gt_bool = (gt_occ > 0.5) & (occ_eval_mask > 0.5)
                    pred_occ_bool = (occ_bin > 0.5) & (occ_eval_mask > 0.5)
                    pred_vis_bool = ((1.0 - occ_bin) > 0.5) & (occ_eval_mask > 0.5)
                ...
                if occ_gt_sem == "visible":
                    pred_prob = (1.0 - occ_prob) * occ_eval_mask
                    pred_bin2 = (1.0 - occ_bin) * occ_eval_mask
                else:
                    pred_prob = occ_prob * occ_eval_mask
                    pred_bin2 = occ_bin * occ_eval_mask

                gt_bool = (gt_occ > 0.5) & (occ_eval_mask > 0.5)
                pred_bool = (pred_bin2 > 0.5) & (occ_eval_mask > 0.5)

                st = bin_stats(pred_bool, gt_bool)
                loss_bce = bce_loss(pred_prob, gt_occ, occ_eval_mask)

                occ_rows.append({"name": name, "cam": cn, **st, "bce": loss_bce})

                # save occ viz
                gt_np = (gt_occ[0, 0].detach().cpu().numpy()).astype(np.float32)
                pr_np = (pred_bin2[0, 0].detach().cpu().numpy()).astype(np.float32)
                mk_np = (occ_eval_mask[0, 0].detach().cpu().numpy()).astype(np.float32)
                gt_np = gt_np * mk_np
                pr_np = pr_np * mk_np
                err_np = np.abs(pr_np - gt_np) * mk_np

                save_uint8_png(occ_root / cn / "gt" / f"{name}.png", (gt_np * 255).astype(np.uint8))
                save_uint8_png(occ_root / cn / "pred" / f"{name}.png", (pr_np * 255).astype(np.uint8))
                save_uint8_png(occ_root / cn / "error" / f"{name}.png", (err_np * 255).astype(np.uint8))
